...
#### Continue...
input_path = paste0(act_tile_path, subpath_modis_temporal_aggregated)
#input_path = "F:/Uni/EnvironmentalObservations/modis_carpathian_mountains/data_small_test/modis/tiles/c0001-0511_r0001-0522/modis_temporal_aggregated"

input_filepath = list.files(input_path, pattern = glob2rx("*.tif"), full.names = TRUE)
output_subdirectory = "output_aggreg"

prefix="testfill"
suffix="123"
outfilepath = compileOutFilePath(input_filepath, output_subdirectory, prefix, suffix)

raster_stack = stack(input_filepath)
fillGapsLin(raster_stack, outfilepath)
